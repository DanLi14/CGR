...
console.log(gameSearchInputField.value);
    ...
    response
      ...
      .then((data) => {
        ...
        for (let review of data) {
          if (gameSearchInputField.value.toUpperCase() === data[i].title.toUpperCase()) {
            createGameCard(review);
            v++;
            i++;
          } else {
            i++;
        ...
        if (v ? '' : alert('Sorry, no game found with that name - please try again.'));
      })
      .catch((error) => {
